...
class CustomDataset(data.Dataset):
    ...
    def __init__(self, img_root, thresh, transform=None, target_transform=None,  # tensor_root
        ...
        self.loader = loader
        self.tensor_folder = '/data1/adamw/HDNeuron/tensors/tensors/'
    ...
    def __getitem__(self, index):
        ...
        img = MapCrop(64, tensor=tensor_sample, thresh=self.thresh)(img)  # <-- TODO: train w/ thresh=0.50
...
class CustomDataset224(data.Dataset):
    """A generic data loader where the images are arranged in this way: ::
    ...
    def __getitem__(self, index):
        ...
        tensor_path = os.path.join(self.tensor_folder, img_name + '.txt')
        # tensor_path, target = self.tensors[index]
        img_sample = Image.open(img_path).convert('RGB')  # .convert('L')
        tensor_sample = Image.fromarray(np.loadtxt(tensor_path))
        img = SmallScale(64)(img_sample)
        img = MapCrop(64, tensor=tensor_sample, thresh=self.thresh)(img)  # <-- TODO: train w/ thresh=0.50
...
class CustomDataset299(data.Dataset):
    """A generic data loader where the images are arranged in this way: ::
...
class CustomDataset_dcgan(data.Dataset):
    """A generic data loader where the images are arranged in this way: ::
